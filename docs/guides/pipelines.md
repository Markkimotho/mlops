...
Pipelines in Nexus **really execute** — they are Prefect flow runs whose steps run
as real work and report back live. There is no simulation.
...
=== "Console"
    ...
    Click a run row to open its detail sheet: a **DAG** laid out in dependency
    layers with per-step status dots, plus a **live log tail**. Cancel and retry
    buttons are there (engineer+).
